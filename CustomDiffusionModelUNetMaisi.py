...
"""
Customized diffusion U-Net implementation based on MONAI's
`DiffusionModelUNetMaisi`. This implementation is used by the Maisi
framework and modifies the original architecture to support
additional experimental functionality required for this project.
The code is derived from MONAI (v1.5.0), with changes to
ensure reproducibility and compatibility with upstream components.
"""
...
__all__ = ["CustomDiffusionModelUNetMaisi"]
...
class DiffusionModelUNetMaisi(nn.Module):
    ...
    def _apply_up_blocks_extract_feature(self, h, emb, context, down_block_res_samples):
        for i, upsample_block in enumerate(self.up_blocks):
            res_samples = down_block_res_samples[-len(upsample_block.resnets) :]
            down_block_res_samples = down_block_res_samples[: -len(upsample_block.resnets)]
            h = upsample_block(hidden_states=h, res_hidden_states_list=res_samples, temb=emb, context=context)
            self.features[f"up_block_{i}"] = h.detach()
    ...
    def forward(
        ...
        Args:
            ...
            extract_feature: If True, extract and store intermediate features.
        Returns:
        ...
        if extract_feature:
            self.features["middle_block"] = h.detach()
        ...
        if extract_feature:
            h = self._apply_up_blocks_extract_feature(h, emb, context, _updated_down_block_res_samples)
        else:
